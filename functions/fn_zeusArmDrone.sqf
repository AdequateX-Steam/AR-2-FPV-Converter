...
switch _mode do
{
	case "init":
	{
		...
		//--- Terminate on client (unless it's curator who created the module)
			...
			//Zeus or Curator activation
			if ((hasInterface && ({local _x} count (objectcurators _logic) >= 1))) then //(_isActivated && _isCuratorPlaced)
			{
	...
	*/
		...
		if (isServer) then {
			////3den Module activation (ingame not editor)
